...
u_max = 5; % [mm], prescribed horizontal displacement.
dt = 0.0008; % [s].
total_t = 1; % [s].
...
% History containers:
u_hist = 0;
horizont_force_hist = 0;

residual_vector = ones(N_dof,1); % Initialise with ones to enter time stepping loop
step = 0;
sign = 1; % Parameter to reverse the displacement after u_max was reached.

while sum(residual_vector(dof_prescr)) >= 0 % Time stepping.
    if step > 2e3
        error('/// Too many steps were made. Exiting...')
    end
    step = step + 1;
%    du = zeros( numel(Dof),1 ); % Do not use results from previous time step.
    if du( dof_prescr(1) )*step == u_max
        sign = -1;
        fprintf('///        Starting to unload.')
    end
    du( bc(:,1) ) = bc(:,2)*sign*dt; % Reason why BC are defined as velocities.
    ...
    % Save history:
    u_hist = [u_hist, u(dof_prescr(1))];
    horizont_force_hist = [horizont_force_hist, sum(residual_vector(dof_prescr))];
...
plot(u_hist, horizont_force_hist,'o-')  
